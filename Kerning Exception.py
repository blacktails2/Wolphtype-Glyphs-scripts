...
class KerningException( object ):
	def __init__( self ):
		...
		windowHeight = spaceY*7+buttonSizeY*6
		self.w = vanilla.FloatingWindow(
			( windowWidth, windowHeight ), # default window size
			"Kerning Exception 2", # window title
		...
		self.w.runButton5 = vanilla.Button((spaceX, spaceY*5+buttonSizeY*4, buttonSizeX, buttonSizeY), u"5 🔓 🔓 Unlock Right2 ", sizeStyle='regular', callback=self.KerningExceptionMain )
		self.w.runButton6 = vanilla.Button((spaceX, spaceY*6+buttonSizeY*5, buttonSizeX, buttonSizeY), u"6 🔒 🔒 Unlock Left2 ", sizeStyle='regular', callback=self.KerningExceptionMain )
		...
		self.w.runButton5.bind('5', [])
		self.w.runButton6.bind('6', [])
	...
	def KerningExceptionMain( self, sender ):
		try:
			...
			nextLayer = View.cachedGlyphAtIndex_(View.activeIndex() + 1)
			...
			if validLayer(activeLayer) and validLayer(prevLayer):
				if sender == self.w.runButton1: #Unlock Right
					...
					if Glyphs.versionNumber >= 3.0:
						...
						activeLayer.setLeftKerningExeption_forLayer_(False, prevLayer)
						prevLayer.setRightKerningExeption_forLayer_(False, activeLayer)
				elif  sender == self.w.runButton5: #Unock Both
					if Glyphs.versionNumber >= 3.0:
						f.setKerningForPair(mID, activeGlyph.name, nextGlyph.name, 0)
					else:
						activeLayer.setRightKerningExeption_forLayer_(True, prevLayer)
						nextLayer.setLeftKerningExeption_forLayer_(False, activeLayer)
				elif  sender == self.w.runButton6: #Unock Both
					if Glyphs.versionNumber >= 3.0:
						f.setKerningForPair(mID, activeGlyph.name, nextGlyph.name, 0)
					else:
						activeLayer.setRightKerningExeption_forLayer_(False, prevLayer)
						nextLayer.setLeftKerningExeption_forLayer_(True, activeLayer)
			else:
				if Glyphs.versionNumber >= 3.0:
